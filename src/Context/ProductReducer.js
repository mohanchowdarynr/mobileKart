...
const ProductReducer = (state, action) => {
   switch(action.type){
       ...
       case saveForLater:{
        const filterCart = state.cartItems.filter((item) => item.itemId !== action.payload.itemId);
           return {
               ...state,
       ...
       case moveToCart :{
        const movedCart = state.saveForLater.filter((item) => item.itemId !== action.payload.itemId);
           return {
               ...state,
