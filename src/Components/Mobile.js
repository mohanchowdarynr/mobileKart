    ...
    return (
        <div className="content border border-gray-600 rounded-2xl border-opacity-10 ">
            ...
            <div className="flex justify-center items-center mt-4">
                <span className="bg-gray-200 px-2 md:px-4 rounded-full text-sm md:text-md mx-1 md:mx-2">₹ {item.price }</span>
                <span className="bg-gray-200 px-4 rounded-full text-xs md:text-sm mx-2">{item.discount}% off</span>
            </div>
            <div className="justify-center mx-auto text-center mt-2">
                 <button onClick={() => addCart(item)} className="bg-gray-700 text-white py-1 px-2 md:px-4 my-2 rounded-full font-medium md:font-bold">Add to cart</button>
            </div>
        </div>
    )
}
